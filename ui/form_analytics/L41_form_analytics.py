...
class C41_FormAnalytics(C20_PySideForm, Ui_form_analytics):
	...
	def InitMenuArchives(self):
		...
		self.action_items_item_expand_include = QAction(icon_item_checked,   "Расширить выборочно")
		self.action_items_item_edit_exclude   = QAction(icon_edit,           "Редактировать")
		self.action_items_item_expand_exclude = QAction(icon_item_unchecked, "Расширить выборочно")
		...
		self.submenu_items_item_include       = self.submenu_items_item.addMenu(icon_item_checked, "Признаки включения")
		self.submenu_items_item_include.addAction(self.action_items_item_edit_include)
		self.submenu_items_item_include.addAction(self.action_items_item_expand_include)

		self.submenu_items_item_exclude       = self.submenu_items_item.addMenu(icon_item_unchecked, "Признаки исключения")
		self.submenu_items_item_exclude.addAction(self.action_items_item_edit_exclude)
		self.submenu_items_item_exclude.addAction(self.action_items_item_expand_exclude)
