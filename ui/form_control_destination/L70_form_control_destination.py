# ФОРМА УПРАВЛЕНИЕ НАЗНАЧЕНИЕМ: МЕХАНИКА УПРАВЛЕНИЯ
...
from L60_form_control_destination import C60_FormControlDestination
from L90_operations               import C90_Operation
from L90_rules                    import C90_ProcessingRule


class C70_FormControlDestination(C60_FormControlDestination):
	""" Форма Управление назначением: Механика управления """

	# Форма
	def ShowTitle(self):
		""" Отображение заголовка формы """
		self.setWindowTitle(f"Управление назначением операций - {self.workspace.DmDyToString()}")
	...
	def AdjustMenuRules_Enable(self):
		""" Меню сопоставления назначения: Настройка доступности """
	...
	def AdjustMenuRules_Text(self):
		""" Меню сопоставления назначения: Настройка текста """
		self.submenu_rules_rule.setTitle("Правило сопоставления назначения")
	...
	def ShowMenuRules(self):
		""" Отображение меню правил сопоставления назначения """
	...
	def FillListControlReplaceAvailable(self):
		...
		descriptions    : list[str] = filter_operations.ToStrings(idp_destination, True, True).data
