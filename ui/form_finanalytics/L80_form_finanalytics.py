...
class C80_FormFinanalytics(C70_FormFinanalytics):
	...
	def CalcDataFindescriptionDynamic(self):
		...
		dialog_progress = QProgressDialog("Динамика финсостава", "Отмена", 0, len(self._idos_findescription_dynamic))
		...
		for findescription_ido in self._idos_findescription_dynamic:
			...
			for index_shift in range(13):
				amounts_income[index_shift]  = self.finstatistic.CalcIncomeByFindescription(findescription_ido, dy, dm)
				amounts_outcome[index_shift] = self.finstatistic.CalcOutcomeByFindescription(findescription_ido, dy, dm)
	...
	def IncFindescriptionDynamic(self):
		...
		findescription_idos  : list[str]        = self.findescription.NamesToIdos(findescription_names)

		self._idos_findescription_dynamic = list(set(self._idos_findescription_dynamic).union(findescription_idos))
	...
	def DecFindescriptionDynamic(self):
		...
		if     self._ido_processing not in self._idos_findescription_dynamic: return

		self._idos_findescription_dynamic.remove(self._ido_processing)
