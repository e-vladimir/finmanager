...
class C80_ProcessingRules(C70_ProcessingRules):
	...
	def IdosByType(self, rule_type: RULES, flag_sort_by_output: bool = False) -> list[str]:
		""" Список IDO по типу правил обработки данных """
		rule        = C80_ProcessingRule()
		idc         = rule.Idc().data
		idp_type    = rule.f_rule_type.Idp().data
		idp_input   = rule.f_input.Idp().data
		idp_output  = rule.f_output.Idp().data

		filter_data = C30_FilterLinear1D(idc)
		filter_data.FilterIdpVlpByEqual(idp_type, rule_type.value)
		filter_data.Capture(CONTAINERS.DISK)

		return filter_data.Idos(idp_output if flag_sort_by_output else idp_input).data
